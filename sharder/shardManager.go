...
func (m *Manager) SetNumShards(n int) {
	...
	defer m.Unlock()
	if m.started {
		logger.Panic("Can't set num shard after started")
	}
...
func (m *Manager) startSession(shard int) error {
	...
	m.handleEvent(EventOpen, shard+1, "")
...
func (m *Manager) LogConnectionEventStd(e *Event) {
	logger.Info(fmt.Sprintf("(Sharder) %s", e.String()))
}
...
//StdGuildCountsFunc uses the standard states to return the guilds
...
const (
	//EventConnected Sent when the connection to the gateway was established
	EventConnected EventType = iota

	//EventDisconnected Sent when the connection is lose
	EventDisconnected

	//EventResumed Sent when the connection was sucessfully resumed
	EventResumed

	//EventReady Sent on ready
	EventReady

	//EventOpen Sent when Open() is called
	EventOpen

	//EventClose Sent when Close() is called
	EventClose

	//EventError Sent when an error occurs
	EventError
)
